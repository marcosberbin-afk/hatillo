<?php
$modulo = 'unidades';
$modulo_titulo = 'Gestión de Unidades';
...
if ($op == '' and $id == '') {
	...
	if ($unidades and $op == 'mod') {

		$op = "mod";
		$titulo = "Modificar Unidad";
		$modificar = 1;
	} elseif ($unidades and $op == 'del') {
		...
		<div class="container-fluid">
			<div class="row">
				<div class="col-12">
					...
					<form action="unidades_operacion.php" method="POST" enctype="multipart/form-data">
						<input type="hidden" name="op" value="<? echo $op; ?>">
						<input type="hidden" name="id" value="<? echo $unidades[0]['hash']; ?>">
								...
								if ($modificar) { ?>
									<div class="text-right">
										...
										if ($op == "add") { ?><button type="submit" class="btn btn-primary"><i class="fas fa-plus"></i> Agregar
												Unidad</button><? } elseif ($op == "mod") { ?><button type="submit"
												class="btn btn-warning"><i class="fas fa-pen"></i> Modificar
												Unidad</button><? } elseif ($op == "del") { ?><button type="submit"
